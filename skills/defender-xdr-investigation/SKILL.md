...
2. **Map coverage.** Select the smallest relevant tables and note what each can and cannot establish. Use `xdr_get_schema` before inventing a table or column, when a bundled pattern does not cover the query, or when tenant drift is plausible. Describing an exact table checks its columns against the tenant by default. Trust those returned columns over the bundled patterns, including any column marked as absent from the documentation snapshot. Treat anything listed under `documentedNotInTenant` as unavailable here, and pass `refresh: true` when a lookup's `liveVerification` looks too old to trust. Every `xdr_run_query` call records the tenant's columns for the tables it read, so once any query has run, a later `xdr_get_schema` search reports what this tenant serves under `cachedTenantMatches` rather than what the documentation claims. Continue when every planned claim has a candidate source or a named coverage gap.
