...
export class SecureCacheUnavailableError extends Error {
  constructor() {
    super(
      "OS-backed secure token storage is unavailable. Run claude-defender-xdr-login interactively to approve an owner-only unencrypted file cache, or fix the OS keychain/secret service.",
    );
...
export interface PersistentCache {
  ...
  security: "os-protected" | "owner-only-file";
  path: string;
}
...
export async function createPersistentCache(
  ...
  try {
    ...
    if (!allowUnencryptedFallback) throw new SecureCacheUnavailableError();
  }
