...
import { getXdrDirectory, loadConfig, type XdrConfig } from "./config.js";
...
async function exportResult(result: unknown): Promise<string> {
...
function errorResult(error: unknown) {
  ...
  return {
    content: [{ type: "text" as const, text: `Defender XDR error: ${text}` }],
    isError: true,
  };
}
...
const config = () =>
  memoize(
    () => loadConfig(),
    () => configPromise,
    value => (configPromise = value),
  );

const auth = () =>
  memoize(
    () => config().then(resolved => XdrAuth.create(resolved)),
    () => authPromise,
    value => (authPromise = value),
  );
...
const server = new McpServer({
  ...
  description:
    "Read-only Microsoft Defender XDR Advanced Hunting via Microsoft Graph. Authentication is never started by tools; run claude-defender-xdr-login first.",
});
...
server.registerTool(
  ...
  {
    description:
      "Run a bounded, read-only KQL query against Microsoft Defender XDR Advanced Hunting. Never writes or changes tenant state. Use claude-defender-xdr-login when authentication is required.",
    inputSchema: {
...
server.registerTool(
  ...
  {
    description:
      "List, search, or describe Defender XDR Advanced Hunting tables and columns from the bundled official schema snapshot. Optionally verify an exact table against the tenant with take 0.",
    inputSchema: {
  ...
  async ({ table, search, live, include_retired, verbose }) => {
    try {
      if (table && search) throw new Error("Specify either table or search, not both");
      if (live && !table) throw new Error("live requires an exact table");
      ...
      if (table) {
        ...
        if (!found) {
          const suggestions = searchSchema(snapshot, table, true, 5)
            ...
            .join(", ");
          throw new Error(
            `Unknown Defender XDR table ${JSON.stringify(table)}; possible matches: ${suggestions}`,
          );
        }
        ...
        let payload: Record<string, unknown> = {
          ...
          columns: found.columns.map(column =>
            verbose ? column : { name: column.name, type: column.type },
          ),
        };
