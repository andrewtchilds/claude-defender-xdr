...
function describeFailure(status, retryAfterSeconds) {
    switch (status) {
        case 400:
        ...
        case 429:
            return `Defender XDR query throttled${retryAfterSeconds === undefined ? "" : `; retry after ${retryAfterSeconds}s`}`;
        default:
            return "Defender XDR query failed";
    }
}
function assertHuntingShape(value) {
    ...
    const badSchema = value.schema.some(column => !column ||
 ...
 */
export async function runHuntingQuery(auth, config, input, signal) {
    if (!input.query.trim())
        throw new Error("Hunting query must not be empty");
    ...
    for (let attempt = 0;; attempt++) {
        ...
        try {
            ...
            if (deadline.aborted || (error instanceof Error && error.name === "AbortError"))
                throw cancelled();
        ...
        if (retryable) {
            ...
            const waitMs = retryAfterSeconds === undefined ? 1000 * 2 ** attempt : retryAfterSeconds * 1000;
        ...
        if (!response.ok) {
            ...
            throw new XdrApiError(`${describeFailure(response.status, retryAfterSeconds)} (${response.status}): ${detail}`, response.status, retryAfterSeconds);
        }
